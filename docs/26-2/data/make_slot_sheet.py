...
from sector_map import build, MEGA, MANDATE, split_sectors, HOLDCO
...
NOTE = {
    1: '담당 산업 6개, 인원 6명으로 1인 1산업이다. 순수지주는 「지주」로 따로 묶고 재료가 같은 것끼리 '
       '하위 테마를 붙였다 — 지주 6종목 중 4종목이 반도체·AI다. SK스퀘어는 SK하이닉스 룩스루 배수가 '
       '1.8배이므로 편입 시 펀드 전체 노출을 함께 계산한다.',
    2: '담당 산업 6개, 인원 5명이므로 팀장이 1개를 겸임한다. 지주 분리로 작아진 소재·화학·철강과 '
       '에너지·상사·운송을 「소재·에너지」로 합쳤다. 두산은 자체 사업(전자BG 하이엔드 CCL)과 '
       '자회사(두산에너빌리티 가스터빈·SMR)가 모두 AI 데이터센터로 모인다.',
...
def main():
    ...
    for t in (1, 2, 3):
        ...
        for line in (
            ...
            '5.  지주는 자체 사업과 보유 자회사 두 갈래를 모두 재료로 본다. 시트에 기입되어 있다.',
     '     편입 전 같은 팀이 보유한 자회사와의 합산 노출을 함께 확인한다.',
        ):
